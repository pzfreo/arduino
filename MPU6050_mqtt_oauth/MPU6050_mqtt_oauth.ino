...
byte server[] = { 
  192,168,1,68 };// 192, 168, 1, 201 };
//byte ip[]     = { 192, 168, 1, 200 };
char refresh[] = "e9648b44b7348bd66c8e336fd99bf5b";
...
void setup() {
  ...
  {
    if (client.connect(clientid, "r", "r")) {
      char topic[15];
      strcpy(topic, "/c/");
      ...
      strcat(message, "\xFF");
      strcat(message, refresh);
      client.publish("/r", message);
    }
  }
}
...
void loop() {
  ...
  if (!publish) {
    ...
    client.loop(); // wait for new bearer token
    return;
  }
